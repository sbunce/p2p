...
{
	std::fstream f((settings::SHARE_DIRECTORY + name).c_str(), std::ios::in);
	if(f.is_open()){
		return;
	}

	f.open((settings::SHARE_DIRECTORY + name).c_str(), std::ios::out | std::ios::binary);
	f.clear();
...
int main()
{
	boost::filesystem::create_directory(settings::SHARE_DIRECTORY.c_str());
	...
	if(!Hash_Tree.create(settings::SHARE_DIRECTORY + "1_block", root_hash)){
		LOGGER; exit(1);
	}
	...
	if(!Hash_Tree.create(settings::SHARE_DIRECTORY + "2_block", root_hash)){
		LOGGER; exit(1);
	}
	...
	if(!Hash_Tree.create(settings::SHARE_DIRECTORY + "3_block", root_hash)){
		LOGGER; exit(1);
	}
	...
	if(!Hash_Tree.create(settings::SHARE_DIRECTORY + "4_block", root_hash)){
		LOGGER; exit(1);
	}
	...
	if(!Hash_Tree.create(settings::SHARE_DIRECTORY + "256_block", root_hash)){
		LOGGER; exit(1);
	}
	...
	if(!Hash_Tree.create(settings::SHARE_DIRECTORY + "257_block", root_hash)){
		LOGGER; exit(1);
	}
