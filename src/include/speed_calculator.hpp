...
{
public:
	speed_calculator(const unsigned & average_seconds_in):
		Recursive_Mutex(new boost::recursive_mutex()),
		average_seconds(average_seconds_in),
		average(0)
	{
		Second_Bytes.push_back(std::make_pair(std::time(NULL), 0));
		for(int x=0; x < average_seconds; ++x){
			Second_Bytes.push_back(std::make_pair(0, 0));
		}
	}
...
private:
	...
	unsigned average_seconds;

	/*
	pair<second, bytes in second>
	The low elements are more current in time.
	*/
	std::vector<std::pair<time_t, unsigned> > Second_Bytes;
};
#endif
