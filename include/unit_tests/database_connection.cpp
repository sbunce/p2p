...
int main()
{
	...
	std::string str = "test";
	if(DB.query("SELECT test_text FROM sqlite3_wrapper", call_back_with_object,
		&str) != SQLITE_OK)
	{
		LOGGER; exit(1);
	}
	...
	test Test;
	if(DB.query("SELECT test_text FROM sqlite3_wrapper", &Test, &test::call_back)
		!= SQLITE_OK)
	{
		LOGGER; exit(1);
	}
	//member function call back with object
	if(DB.query("SELECT test_text FROM sqlite3_wrapper", &Test,
		&test::call_back_with_object, &str) != SQLITE_OK)
	{
		LOGGER; exit(1);
	}
}
