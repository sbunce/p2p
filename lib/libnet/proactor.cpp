...
void net::proactor::add_socket(std::pair<int, boost::shared_ptr<connection> > P)
{
	assert(P.first != -1);
	if(P.second){
		assert(P.second->N);
...
void net::proactor::adjust_connection_limits(const unsigned incoming_limit,
...
void net::proactor::append_send_buf(const int connection_ID,
...
void net::proactor::check_timeouts()
{
	/*
...
void net::proactor::connect(const endpoint & ep)
{
	...
	relay_job.push_back(boost::bind(&proactor::connect_relay, this, ep));
}

void net::proactor::connect_relay(const endpoint ep)
{
	...
	if(Connection->socket() != -1){
		...
		{//BEGIN lock scope
		boost::mutex::scoped_lock lock(relay_job_mutex);
		relay_job.push_back(boost::bind(&proactor::add_socket, this, P));
		}//END lock scope
...
{
	boost::mutex::scoped_lock lock(relay_job_mutex);
	relay_job.push_back(boost::bind(&proactor::disconnect, this,
		connection_ID, false));
	Select.interrupt();
}

void net::proactor::disconnect(const int connection_ID, const bool on_empty)
{
	std::pair<int, boost::shared_ptr<connection> > P = lookup_ID(connection_ID);
	if(P.second){
		if(on_empty){
...
void net::proactor::handle_async_connection(
...
std::pair<int, boost::shared_ptr<net::connection> >
	net::proactor::lookup_socket(const int socket_FD)
{
...
std::pair<int, boost::shared_ptr<net::connection> >
	net::proactor::lookup_ID(const int connection_ID)
{
	std::map<int, boost::shared_ptr<connection> >::iterator
...
void net::proactor::network_loop()
{
	//used to run tasks once per second
	std::time_t network_loop_time(std::time(NULL));
	while(true){
...
void net::proactor::process_relay_job()
{
	while(true){
...
void net::proactor::remove_socket(const int socket_FD)
{
	assert(socket_FD != -1);
...
void net::proactor::send(const int connection_ID, buffer & send_buf)
{
	if(!send_buf.empty()){
		...
		relay_job.push_back(boost::bind(&proactor::append_send_buf, this,
			connection_ID, buf));
		Select.interrupt();
	}
}
	...
	const unsigned outgoing_limit)
{
	...
	boost::mutex::scoped_lock lock(relay_job_mutex);
	relay_job.push_back(boost::bind(&proactor::adjust_connection_limits,
...
void net::proactor::start(boost::shared_ptr<listener> Listener_in)
{
	...
	network_loop_thread = boost::thread(boost::bind(&proactor::network_loop, this));
}
