...
database::blob database::table::hash::tree_open(const std::string & hash, const int & tree_size)
{
	database::connection DB;
	return tree_open(DB, hash, tree_size);
}

database::blob database::table::hash::tree_open(database::connection & DB, const std::string & hash, const int & tree_size)
{
	std::pair<bool, boost::int64_t> info;
	std::stringstream ss;
