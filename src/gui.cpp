...
gui::gui() : Gtk::Window(Gtk::WINDOW_TOPLEVEL)
{
	...
	notebook->append_page(*GUI_Window_Download, *download_label);
	notebook->pages().back().set_tab_label_packing(false, true, Gtk::PACK_START);
	notebook->append_page(*GUI_Window_Upload, *upload_label);
	notebook->pages().back().set_tab_label_packing(false, true, Gtk::PACK_START);
...
void gui::help_about()
{
	boost::shared_ptr<gui_about> GUI_About(new gui_about());
